...
class SelectDirectoryAdapter(var currentDirectory : String) : RecyclerView.Adapter<SelectDirectoryAdapter.SelectDirectoryViewHolder>()
{
    ...
    data class Folder(val folderName : String, val folderPath : String, val isParent : Boolean = false)
    ...
    {
        folders.clear()
        File(currentDirectory).listFiles()?.forEach { if(it.isDirectory){ folders.add(Folder(it.name, it.absolutePath)) } }
    ...
    {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.listviewitem_directory, parent, false)
